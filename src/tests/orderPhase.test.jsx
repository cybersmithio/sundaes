...
test('Order phases for happy path', async() => {
    ...
    const cherryTopping = await screen.findByRole("checkbox", {name: /cherries/i});
    ...
    const pageTitle = await screen.findByRole("heading", {name: /order summary/i});
    expect(pageTitle).toBeInTheDocument();

    const scoopTotal = await screen.findByRole("heading", {name: /scoops: \$/i});
    expect(scoopTotal).toHaveTextContent('6.00');

    const toppingTotal = await screen.findByRole("heading", {name: /toppings: \$/i});
    expect(toppingTotal).toHaveTextContent('1.50');

    const orderSummaryTotal = await screen.findByRole("heading", {name: /grand total: \$/i});
    ...
    const agreementCheckbox =  screen.getByRole("checkbox", {name: /i agree to/i});
    ...
    // start a new order
    const createNewOrderButton = await screen.findByRole("button", {name: /create new order/i});
    userEvent.click(createNewOrderButton);

    // make sure order page is reset
    const orderEntryTitle = await screen.findByRole("heading", {name: /design your sundae!/i});
    expect(orderEntryTitle).toBeVisible();
    const newGrandTotal = await screen.findByRole("heading", {name: /grand total: \$/i});
